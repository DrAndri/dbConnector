...
(function($) {
   ...
   $.widget("ui.weekCalendar", {
      ...
      today : function() {
         this._clearCalendar();
         var date = new Date();
         this.options.firstDayOfWeek = date.getDay() - 1;
      ...
      prevWeek : function() {
         ...
         //var daysToScroll = this.options.daysToShow > 7?7:this.options.daysToShow;
         var newDate = new Date(this.element.data("startDate").getTime() - MILLIS_IN_DAY*this.options.daysToShow);
         this.options.firstDayOfWeek = (this.options.firstDayOfWeek - this.options.daysToShow)%7;
         this._clearCalendar();
         this._loadCalEvents(newDate);
         ga('send', {
           hitType: 'event',
           eventCategory: 'Navigation',
           eventAction: 'prevWeek'
         });
      },
      ...
      getTimeslotTimes : function(date) {
         ...
         for (var i = 0; i < options.timeslotsPerDay; i++) {
            ...
            times[i] = {
               ...
               startFormatted: this._formatDate(new Date(startMillis), options.dateTimeFormat),
               end: new Date(endMillis),
               endFormatted: this._formatDate(new Date(endMillis), options.dateTimeFormat)
            };
      ...
      _resizeCalendar : function () {
         ...
         if (options && $.isFunction(options.height)) {
            ...
            this.element.find(".wc-scrollable-grid").height(calendarHeight - navHeight);
         }
      },
      ...
      _renderCalendar : function() {
         ...
         if (options.buttons) {
            ...
            $calendarContainer.find(".wc-nav .wc-todayBtn").click(function() {
      ...
      createNewEvent : function() {
          ga('send', {
            ...
            eventAction: 'newEvent - plus-button'
          });
          this.options.eventNew();
      },
      ...
      _updateDayColumnHeader : function ($weekDayColumns) {
         ...
         $weekDayColumns.each(function(i, val) {
            ...
            var clonedDate = self._cloneDate(currentDay);
            if(new Date().getHours() < self.options.businessHours.start)
                clonedDate.setDate(clonedDate.getDate() +1);
            if (self._isToday(clonedDate)) {
      ...
      _findWeekDayForEvent : function(calEvent, $weekDayColumns) {
         ...
         var options = this.options;
         $weekDayColumns.each(function() {
            if ($(this).data("startDate").getTime() <= calEvent.start.getTime() + options.businessHours.start * 3600000 && $(this).data("endDate").getTime() + options.businessHours.start * 3600000 >= calEvent.end.getTime()) {
               $weekDay = $(this);
               return false;
            }
         });
      ...
      _positionEvent : function($weekDay, $calEvent) {
         ...
         //add a whole day if startMillis is negative
         //for events that start after midnight
         if(startMillis < 0)
      ...
      _addDraggableToCalEvent : function(calEvent, $calEvent) {
         ...
         $calEvent.draggable({
            ...
            //containment: "window",
            containment: [$(".wc-scrollable-grid").position().left, $(".wc-scrollable-grid").position().top, $(".wc-scrollable-grid").position().right, windowHeight],
            scrollSensitivity: 1,
            revert: 'valid',
            opacity: 0.5,
      ...
      setDaysToShow: function(daysToShow) {
          var self = this;
          self.options.daysToShow = parseInt(daysToShow);
          $(self.element).html('');
          self._renderCalendar();
          self._loadCalEvents();
          self._resizeCalendar();
      ...
      setOverflowHours: function(overflowHours) {
          ...
          self.options.overflowHours = overflowHoursInt;
          self.options.businessHours.start = overflowHoursInt;
          self.options.businessHours.end = 24 + overflowHoursInt;
          $(self.element).html('');
          self._renderCalendar();
          self._loadCalEvents();
          self._resizeCalendar();
      ...
      scrollToCurrentHour : function() {
         var options = this.options;
         var $scrollable = this.element.find(".wc-scrollable-grid");
         var date = new Date();
         var slot = date.getHours();
         if (options.businessHours.limitDisplay) {
            if (date.getHours() < options.overflowHours) {
                slot += 24 - options.overflowHours - options.scrollToCurrentHourSubtract;
            } else {
                slot -= options.overflowHours + options.scrollToCurrentHourSubtract;
            }
         }
         var $target = this.element.find(".wc-grid-timeslot-header .wc-hour-header:eq(" + slot + ")");
         $scrollable.animate({scrollTop: 0}, 0, function() {
      ...
      _dateFirstDayOfWeek : function(date) {
         ...
         adjustedDate.setHours(self.options.overflowHours);
         return adjustedDate;
      ...
      _dateLastDayOfWeek : function(date) {
         var self = this;
         var midnightCurrentDate = new Date(date.getFullYear(), date.getMonth(), date.getDate());
         midnightCurrentDate.setHours(self.options.overflowHours);
         var adjustedDate = new Date(midnightCurrentDate);
         adjustedDate.setDate(adjustedDate.getDate() + self.options.daysToShow - 1);
         return adjustedDate;
      },
   ...
   $.extend($.ui.weekCalendar, {
      ...
      defaults: {
         ...
         timeFormat : "H:i",
         dateFormat : "M d, Y",
         dateTimeFormat : "H:i d M",
         use24Hour : true,
         daysToShow : Math.round(window.innerWidth/220),
